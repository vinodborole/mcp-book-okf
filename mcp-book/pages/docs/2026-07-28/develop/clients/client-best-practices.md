...
resource: https://modelcontextprotocol.io/docs/2026-07-28/develop/clients/client-best-practices
...
**Layer 1: Catalog.**The host exposes a small set of meta-tools for searching available capabilities. A
...
[agent skills](/docs/2026-07-28/develop/build-with-agent-skills), a skill file can declare which MCP servers it needs, and the host connects them only when that skill is invoked.
...
specification’s [caching utility](/specification/2026-07-28/server/utilities/caching). In particular, treat a cached list as stale once a

`list_changed` notification arrives, even before its TTL
expires.
...
**Step 1: Generate a programmatic API from MCP schemas.**The host reads each server’s tool definitions and produces typed functions based on each tool’s arguments and

`outputSchema`:
[for each tool. When an output schema is present, the host can produce precise return types (like](/specification/2026-07-28/server/tools#output-schema)
...
[rather than a transport failure. Generated wrappers should convert this into a thrown exception so model-authored code can use](/specification/2026-07-28/server/tools#error-handling)
...
Progressive discovery and programmatic tool calling work well together. The model uses discovery tools to identify which tools it needs, loads their schemas, and then writes a single script that calls multiple tools in one execution pass. This combination minimizes both the token cost of tool definitions
*and*the token cost of tool results, keeping the model’s context focused on reasoning rather than passing data through it.

# Citations

1. Source page: https://modelcontextprotocol.io/docs/2026-07-28/develop/clients/client-best-practices
