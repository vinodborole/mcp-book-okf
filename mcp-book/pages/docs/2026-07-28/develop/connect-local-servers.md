...
resource: https://modelcontextprotocol.io/docs/2026-07-28/develop/connect-local-servers
...
[Claude Desktop](https://claude.ai/download)for your operating system. Claude Desktop is available for macOS and Windows. If you already have Claude Desktop installed, verify you’re running the latest version by clicking the Claude menu and selecting “Check for Updates…”
...
[nodejs.org](https://nodejs.org/). We recommend the LTS (Long Term Support) version for stability.
...
`username` with your actual computer username. The paths listed in the `args` array specify which directories the Filesystem Server can access. You can modify these paths or add additional directories as needed.
4
...
[Troubleshooting](#troubleshooting)section for debugging steps.
...
`${APPDATA}` within a path, you may need to add the expanded value of `%APPDATA%` to your `env` key in `claude_desktop_config.json`:
## Next Steps
...
1. Source page: https://modelcontextprotocol.io/docs/2026-07-28/develop/connect-local-servers
